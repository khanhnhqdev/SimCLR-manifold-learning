...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20', 'cifar-100', 'caltech-101']:
    ...
    # Setup
    if p['setup'] in ['simclr', 'moco', 'simclr-random-w', 'simclr-uniform-w', 'simclr-LLE', 'simclr-random-w-C', 'finetune']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False, to_many_views=False,
    ...
    elif p['train_db_name'] == 'caltech-101':
        from data.caltech import CALTECH101
        dataset = CALTECH101(train=True, transform=transform)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    if p['val_db_name'] == 'cifar-10':
        ...
        from data.caltech import CALTECH101
        dataset = CALTECH101(train=False, transform=transform)
